...
class AudioPlayer {
  ...
  /// Set audio info.
  Future<void> setPlaybackInfo( int duration) async =>
      await _channel.invokeMethod('setPlaybackInfo', { 'duration': _duration});

  Future<void> setInfo(String name, String author, String imageUrl, int duration) async =>
      await _channel.invokeMethod('setInfo', {'name':name, 'author':author, 'imageUrl':imageUrl, 'duration': duration});

  Future<void> setDuration( int duration) async =>
      await _channel.invokeMethod('setDuration', {'duration': duration});
  ...
  Future<void> _audioPlayerStateChange(MethodCall call) async {
    switch (call.method) {
      ...
      case "audio.onPause":
        ...
        break;
      case "audio.onSELPause":
        _state = AudioPlayerState.SELPAUSE;
        _playerStateController.add(AudioPlayerState.SELPAUSE);
        await _channel.invokeMethod('setPlaybackInfo', { 'duration': _duration});
        break;
      case "audio.onContinue":
        _state = AudioPlayerState.SELCONTINUE;
        _playerStateController.add(AudioPlayerState.SELCONTINUE);
        await _channel.invokeMethod('setPlaybackInfo', { 'duration': _duration});
        break;
      case "audio.onStop":
      ...
      case "audio.onComplete":
        ...
        await _channel.invokeMethod('setPlaybackInfo', { 'duration': 0});
        break;
      case "audio.onPrevious":
        _state = AudioPlayerState.SELPREVIOUS;
        _playerStateController.add(AudioPlayerState.SELPREVIOUS);
        await _channel.invokeMethod('setPlaybackInfo', { 'duration': 0});
        break;
      case "audio.onError":
